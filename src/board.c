...
#define L_SUP CODER /* if not already defined */
...
{ "Ideas",	  "Suggestion for improvement",	    0,	1,	    "all", DEF_NORMAL,   60, NULL, FALSE },
...
{ "Stories",  "In-game literature",             0,  1,      "all", DEF_INCLUDE, 120, NULL, FALSE },
{ "History",  "Character histories",            0,  1,      "all", DEF_INCLUDE, 120, NULL, FALSE },
{ "Immortal", "Immortal discussion",		L_IMM,	L_IMM,	"imm", DEF_NORMAL,   60, NULL, FALSE },
...
void finish_note (BOARD_DATA *board, NOTE_DATA *note)
{
	...
	if (!fp)
	{
		logstr (LOG_BUG, "Could not open one of the note files in append mode",0);
...
static void unlink_note (BOARD_DATA *board, NOTE_DATA *note)
{
	...
	else
	{
		...
		if (!p)
			logstr (LOG_BUG, "unlink_note: could not find note.",0);
		else
			p->next = note->next;
	}
}
...
static void save_board (BOARD_DATA *board)
{
	...
	if (!fp)
	{
		logstr (LOG_BUG, "Error writing to: %s", filename);
	}
	else
	{
...
static void load_board (BOARD_DATA *board)
{
    ...
    for ( ; ; )
    {
        ...
        if (pnote->expire < current_time)
        {
			...
			if (!fp_archive)
				logstr (LOG_BUG, "Could not open archive boards for writing",0);
			else
			{
    ...
    logstr (LOG_BUG,  "Load_notes: bad key word.", 0 );
    return; /* just return */
}
...
bool is_note_to (CHAR_DATA *ch, NOTE_DATA *note)
{
	...
	if (is_number(note->to_list) && ch->level >= atoi(note->to_list))
		return TRUE;
...
static void do_nwrite (CHAR_DATA *ch, char *argument)
{
	...
	if (ch->level < ch->pcdata->board->write_level)
	{
		sendch ("You cannot post notes on this board.\n\r",ch);
		return;
	}
	...
	{
		sendch ("Note in progress cancelled because you did not manage to write any text \n\r"
		              "before losing link.\n\r\n\r",ch);
	...
	else /* we are continuing, print out all the fields and the note so far*/
	{
		...
		sendch ("{GYour note so far:{x\n\r", ch);
		sendch (ch->pcdata->in_progress->text,ch);

		sendch ("\n\rEnter text. Type {W~{x or {WEND{x on an empty line to end note.\n\r"
                            "Type {WDEL{x on an empty line to remove the previous line.\n\r"
		                    "========================================================\n\r",ch);
...
static void do_nread (CHAR_DATA *ch, char *argument)
{
	...
	else /* just next one */
	{
		...
		sendch ("No new notes in this board.\n\r",ch);
...
static void do_nremove (CHAR_DATA *ch, char *argument)
{
	...
	{
		sendch ("You are not authorized to remove this note.\n\r",ch);
		return;
	}
...
static void do_ncatchup (CHAR_DATA *ch, char *argument)
{
	...
	if (!p)
		sendch ("Alas, there are no notes in that board.\n\r",ch);
	else
	{
		ch->pcdata->last_note[board_number(ch->pcdata->board)] = p->date_stamp;
		sendch ("All mesages skipped.\n\r",ch);
	}
}
...
void do_board (CHAR_DATA *ch, char *argument)
{
	...
	if (!argument[0]) /* show boards */
	{
		...
		count = 1;
		sendch ("{RNum          Name Unread Description{x\n\r"
		              "{R==== ============ ====== ============================={x\n\r",ch);
		for (i = 0; i < MAX_BOARD; i++)
		{
		...
		/* Inform of rights */
		if (ch->pcdata->board->read_level > ch->level)
			sendch ("You cannot read nor write notes on this board.\n\r",ch);
		else if (ch->pcdata->board->write_level > ch->level)
			sendch ("You can only read notes from this board.\n\r",ch);
		else
			sendch ("You can both read and write on this board.\n\r",ch);
    ...
    if (ch->pcdata->in_progress)
    {
    	sendch ("Please finish your interrupted note first.\n\r",ch);
        return;
    }
...
void make_note (const char* board_name, const char *sender, const char *to, const char *subject, const int expire_days, const char *text)
{
	...
	{
		logstr (LOG_BUG, "make_note: board not found",0);
		return;
	}

	if (strlen(text) > MAX_NOTE_TEXT)
	{
		logstr (LOG_BUG, "make_note: text too long (%d bytes)", strlen(text));
		return;
	}
...
void handle_con_note_to (DESCRIPTOR_DATA *d, char * argument)
{
	...
	if (!ch->pcdata->in_progress)
	{
		...
		logstr (LOG_BUG, "nanny: In CON_NOTE_TO, but no note in progress",0);
		return;
	}
...
void handle_con_note_subject (DESCRIPTOR_DATA *d, char * argument)
{
	...
	{
		d->connected = CON_PLAYING;
		logstr (LOG_BUG, "nanny: In CON_NOTE_SUBJECT, but no note in progress",0);
		return;
	}
...
void handle_con_note_expire(DESCRIPTOR_DATA *d, char * argument)
{
	...
	{
		d->connected = CON_PLAYING;
		logstr (LOG_BUG, "nanny: In CON_NOTE_EXPIRE, but no note in progress",0);
		return;
	}
...
void handle_con_note_text (DESCRIPTOR_DATA *d, char * argument)
{
	...
	if (!ch->pcdata->in_progress) {
		d->connected = CON_PLAYING;
		logstr (LOG_BUG, "nanny: In CON_NOTE_TEXT, but no note in progress",0);
		return;
	}
    ...
    else if (!str_cmp(buf, "DEL")) {
        bool bFound = FALSE;
        int i;
        if (!ch->pcdata->in_progress->text) {
            send_to_desc ("\n\rYou don't have anything to delete!\n\r",d);
            send_to_desc ("{gText of your note so far:{x\n\r",d);
            return;
        }
        // Look for the last '\r'
        for (i = strlen(ch->pcdata->in_progress->text); i >= 0; --i) {
            if (ch->pcdata->in_progress->text[i] == '\n') {
                if (bFound) {
                    ch->pcdata->in_progress->text[i+1] = '\0'; // After the end of line
                    send_to_desc ("\n\r{gText of your note so far:{x\n\r",d);
...
void handle_con_note_finish (DESCRIPTOR_DATA *d, char * argument)
{
	...
	CHAR_DATA *ch = d->character;
		...
		{
			d->connected = CON_PLAYING;
			logstr (LOG_BUG, "nanny: In CON_NOTE_FINISH, but no note in progress",0);
			return;
		}
