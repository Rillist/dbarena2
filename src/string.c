...
{
    sendch ("-========- Entering EDIT Mode -=========-\n\r", ch);
    sendch ("    Type .h on a new line for help\n\r", ch);
    sendch (" Terminate with a ~ or @ on a blank line.\n\r", ch);
    sendch ("-=======================================-\n\r", ch);
...
void string_add (CHAR_DATA * ch, char *argument)
{
    ...
    if (*argument == '.')
    {
        ...
        if (!str_cmp (arg1, ".s"))
        {
            sendch ("String so far:\n\r", ch);
            sendch (numlines (*ch->desc->pString), ch);
            return;
        }
        ...
        if (!str_cmp (arg1, ".r"))
        {
            if (arg2[0] == '\0')
            {
                sendch ("usage:  .r \"old string\" \"new string\"\n\r",
                              ch);
                return;
            }
        ...
        if (!str_cmp (arg1, ".lr"))
        {
            ...
            *ch->desc->pString =
                string_lineadd (*ch->desc->pString, tmparg3, atoi (arg2));
            sendch ("Line replaced.\n\r", ch);
            return;
        }

        if (!str_cmp (arg1, ".h"))
        {
            sendch ("Sedit help (commands on blank line):   \n\r", ch);
            sendch (".r 'old' 'new'   - replace a substring \n\r", ch);
            sendch ("                   (requires '', \"\") \n\r", ch);
            sendch (".h               - get help (this info)\n\r", ch);
            sendch (".s               - show string so far  \n\r", ch);
            sendch (".f               - (word wrap) string  \n\r", ch);
            sendch (".c               - clear string so far \n\r", ch);
            sendch (".ld <num>        - delete line number <num>\n\r",
                          ch);
            sendch (".li <num> <str>  - insert <str> at line <num>\n\r",
                          ch);
            sendch
        ...
        sendch ("SEdit:  Invalid dot command.\n\r", ch);
        return;
    }
    ...
    {
        sendch ("String too long, last line skipped.\n\r", ch);
