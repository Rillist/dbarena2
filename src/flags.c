...
void do_flag (CHAR_DATA * ch, char *argument)
{
    ...
    {
        sendch ("Syntax:\n\r", ch);
        sendch ("  flag mob  <name> <field> <flags>\n\r", ch);
        sendch ("  flag char <name> <field> <flags>\n\r", ch);
        sendch ("  mob  flags: act,aff,off,imm,res,vuln,form,part\n\r",
                      ch);
        sendch ("  char flags: plr,comm,aff,imm,res,vuln,\n\r", ch);
        sendch ("  +: add flag, -: remove flag, = set equal to\n\r",
                      ch);
        sendch ("  otherwise flag toggles the flags listed.\n\r", ch);
        return;
    }

    if (arg2[0] == '\0')
    {
        sendch ("What do you wish to set flags on?\n\r", ch);
        return;
    }

    if (arg3[0] == '\0')
    {
        sendch ("You need to specify a flag to set.\n\r", ch);
        return;
    }

    if (argument[0] == '\0')
    {
        sendch ("Which flags do you wish to change?\n\r", ch);
        return;
    }
    ...
    if (!str_prefix (arg1, "mob") || !str_prefix (arg1, "char"))
    {
        ...
        else if (!str_prefix (arg3, "form"))
        {
            if (!IS_NPC (victim))
            {
                sendch ("Form can't be set on PCs.\n\r", ch);
                return;
            }
        ...
        else if (!str_prefix (arg3, "parts"))
        {
            if (!IS_NPC (victim))
            {
                sendch ("Parts can't be set on PCs.\n\r", ch);
                return;
            }
        ...
        else if (!str_prefix (arg3, "comm"))
        {
            if (IS_NPC (victim))
            {
                sendch ("Comm can't be set on NPCs.\n\r", ch);
                return;
            }
        ...
        {
            sendch ("That's not an acceptable flag.\n\r", ch);
            return;
        }
        ...
        for (;;)
        {
            ...
            if (pos == NO_FLAG)
            {
                sendch ("That flag doesn't exist!\n\r", ch);
                return;
            }
            else
                SET_BIT (marked, pos);
