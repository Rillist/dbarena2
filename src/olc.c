...
/*  {   command        function    }, */
	...
	{"dex", medit_dex},
	{"cha", medit_cha},
	{"hitbonus", medit_hitbonus},
	{"kibonus", medit_kibonus},
...
void aedit (CHAR_DATA * ch, char *argument)
{
    ...
    {
        sendch ("AEdit:  Insufficient security to modify area.\n\r",
                      ch);
        edit_done (ch);
        return;
    }
...
void redit (CHAR_DATA * ch, char *argument)
{
    ...
    if (!IS_BUILDER (ch, pArea))
    {
        sendch ("REdit:  Insufficient security to modify room.\n\r",
                      ch);
        edit_done (ch);
        return;
    }
...
void oedit (CHAR_DATA * ch, char *argument)
{
    ...
    if (!IS_BUILDER (ch, pArea))
    {
        sendch ("OEdit: Insufficient security to modify area.\n\r", ch);
        edit_done (ch);
        return;
    }
...
void medit (CHAR_DATA * ch, char *argument)
{
    ...
    if (!IS_BUILDER (ch, pArea))
    {
        sendch ("MEdit: Insufficient security to modify area.\n\r", ch);
        edit_done (ch);
        return;
    }
...
void do_aedit (CHAR_DATA * ch, char *argument)
{
    ...
    if (is_number (arg))
    {
        ...
        {
            sendch ("That area vnum does not exist.\n\r", ch);
            return;
        }
    }
    else if (!str_cmp (arg, "create"))
    {
        ...
        {
            sendch ("AEdit : Insufficient security to create area.\n\r",
                          ch);
            return;
        }
    ...
    if (!IS_BUILDER (ch, pArea))
    {
        sendch ("Insufficient security to edit areas.\n\r", ch);
        return;
    }
...
void do_redit (CHAR_DATA * ch, char *argument)
{
    ...
    {                            /* redit reset */
        ...
        {
            sendch ("Insufficient security to modify room.\n\r", ch);
            return;
        }
    ...
    {                            /* redit create <vnum> */
        ...
        {
            sendch ("Syntax:  edit room create [vnum]\n\r", ch);
            return;
        }
...
void do_oedit (CHAR_DATA * ch, char *argument)
{
    ...
    if (is_number (arg1))
    {
        ...
        {
            sendch ("OEdit:  That vnum does not exist.\n\r", ch);
            return;
        }

        if (!IS_BUILDER (ch, pObj->area))
        {
            sendch ("Insufficient security to modify objects.\n\r", ch);
            return;
        }
    ...
    else
    {
        if (!str_cmp (arg1, "create"))
        {
            ...
            {
                sendch ("Syntax:  edit object create [vnum]\n\r", ch);
                return;
            }
    ...
    sendch ("OEdit:  There is no default object to edit.\n\r", ch);
    return;
}
...
void do_medit (CHAR_DATA * ch, char *argument)
{
    ...
    if (is_number (arg1))
    {
        ...
        {
            sendch ("MEdit:  That vnum does not exist.\n\r", ch);
            return;
        }

        if (!IS_BUILDER (ch, pMob->area))
        {
            sendch ("Insufficient security to modify mobs.\n\r", ch);
            return;
        }
    ...
    else
    {
        if (!str_cmp (arg1, "create"))
        {
            ...
            if (arg1[0] == '\0' || value == 0)
            {
                sendch ("Syntax:  edit mobile create [vnum]\n\r", ch);
                return;
            }
    ...
    sendch ("MEdit:  There is no default mobile to edit.\n\r", ch);
    return;
}
...
void do_resets (CHAR_DATA * ch, char *argument)
{
    ...
    {
        sendch ("Resets: Invalid security for editing this area.\n\r",
                      ch);
        return;
    }
    ...
    if (arg1[0] == '\0')
    {
        ...
        {
            sendch ("Resets: M = mobile, R = room, O = object, "
                          "P = pet, S = shopkeeper\n\r", ch);
            display_resets (ch);
        }
        else
            sendch ("No resets in this room.\n\r", ch);
    }
    ...
    if (is_number (arg1))
    {
                ...
                || (!str_cmp (arg2, "obj") && is_number (arg3)))
        {
            ...
            if (!str_cmp (arg2, "obj"))
            {
                ...
                if (!str_prefix (arg4, "inside"))
                {
                    ...
                    {
                        sendch ("Object 2 is not a container.\n\r", ch);
                        return;
                    }
                    pReset->command = 'P';
        ...
        else if (!str_cmp (arg2, "random") && is_number (arg3))
        {
            ...
            add_reset (ch->in_room, pReset, atoi (arg1));
            SET_BIT (ch->in_room->area->area_flags, AREA_CHANGED);
            sendch ("Random exits reset added.\n\r", ch);
        }
        else
        {
            sendch ("Syntax: RESET <number> OBJ <vnum> <wear_loc>\n\r", ch);
            sendch ("        RESET <number> OBJ <vnum> inside <vnum> [limit] [count]\n\r", ch);
            sendch ("        RESET <number> OBJ <vnum> room\n\r", ch);
            sendch ("        RESET <number> MOB <vnum> [max #x area] [max #x room]\n\r", ch);
            sendch ("        RESET <number> DELETE\n\r", ch);
            sendch ("        RESET <number> RANDOM [#x exits]\n\r", ch);
        }
    }
...
void do_alist (CHAR_DATA * ch, char *argument)
{
    ...
    sprintf (result, "[%3s] [%-27s] (%-5s-%5s) [%-10s] %3s [%-20s]\n\r",
    ...
    {
        sprintf (buf,
                 "[%3d] %-29.29s (%-5d-%5d) %-12.12s [%d] [%-20.20s]\n\r",
