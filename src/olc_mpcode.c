...
void mpedit( CHAR_DATA *ch, char *argument)
{
	...
	if ( !IS_BUILDER(ch, ad) )
	{
		sendch("MPEdit: Insufficient security to modify code.\n\r", ch);
		edit_done(ch);
		return;
	}
    }
...
void opedit( CHAR_DATA *ch, char *argument)
{
	...
	if ( !IS_BUILDER(ch, ad) )
	{
		sendch("OPEdit: Insufficient security to modify code.\n\r", ch);
		edit_done(ch);
		return;
	}
    }
...
void rpedit( CHAR_DATA *ch, char *argument)
{
	...
	if ( !IS_BUILDER(ch, ad) )
	{
		sendch("RPEdit: Insufficient security to modify code.\n\r", ch);
		edit_done(ch);
		return;
	}
    }
...
void do_mpedit(CHAR_DATA *ch, char *argument)
{
	...
	{
		sendch("MPEdit : That vnum does not exist.\n\r",ch);
		return;
	}
	...
	if ( ad == NULL )
	{
		sendch( "MPEdit : Vnum is not assigned an area.\n\r", ch );
		return;
	}

	if ( !IS_BUILDER(ch, ad) )
	{
		sendch("MPEdit : Insufficient security to modify area.\n\r", ch );
		return;
	}
	...
	if (argument[0] == '\0')
	{
		sendch( "Syntax : mpedit create [vnum]\n\r", ch );
		return;
	}
    ...
    sendch( "Syntax : mpedit [vnum]\n\r", ch );
    sendch( "         mpedit create [vnum]\n\r", ch );
...
void do_opedit(CHAR_DATA *ch, char *argument)
{
	...
	{
		sendch("OPEdit : That vnum does not exist.\n\r",ch);
		return;
	}
	...
	if ( ad == NULL )
	{
		sendch( "OPEdit : Vnum is not assigned an area.\n\r", ch );
		return;
	}

	if ( !IS_BUILDER(ch, ad) )
	{
		sendch("OPEdit : Insufficient security to modify area.\n\r", ch );
		return;
	}
	...
	if (argument[0] == '\0')
	{
		sendch( "Syntax : opedit create [vnum]\n\r", ch );
		return;
	}
    ...
    sendch( "Syntax : opedit [vnum]\n\r", ch );
    sendch( "         opedit create [vnum]\n\r", ch );
...
void do_rpedit(CHAR_DATA *ch, char *argument)
{
	...
	{
		sendch("RPEdit : That vnum does not exist.\n\r",ch);
		return;
	}
	...
	if ( ad == NULL )
	{
		sendch( "RPEdit : Vnum is not assigned an area.\n\r", ch );
		return;
	}

	if ( !IS_BUILDER(ch, ad) )
	{
		sendch("RPEdit : Insufficient security to modify area.\n\r", ch );
		return;
	}
	...
	if (argument[0] == '\0')
	{
		sendch( "Syntax : rpedit create [vnum]\n\r", ch );
		return;
	}
    ...
    sendch( "Syntax : rpedit [vnum]\n\r", ch );
    sendch( "         rpedit create [vnum]\n\r", ch );
...
MPEDIT (mpedit_create)
{
    ...
    if ( !IS_BUILDER(ch, ad) )
    {
        sendch("MPEdit : Insufficient security to create MobProgs.\n\r", ch);
        return FALSE;
    }

    if ( get_prog_index(value,PRG_MPROG) )
    {
	sendch("MPEdit: Code vnum already exists.\n\r",ch);
	return FALSE;
    }
    ...
    sendch("MobProgram Code Created.\n\r",ch);
...
OPEDIT (opedit_create)
{
    ...
    if ( !IS_BUILDER(ch, ad) )
    {
        sendch("OPEdit : Insufficient security to create ObjProgs.\n\r", ch);
        return FALSE;
    }

    if ( get_prog_index(value,PRG_OPROG) )
    {
	sendch("OPEdit: Code vnum already exists.\n\r",ch);
	return FALSE;
    }
    ...
    sendch("ObjProgram Code Created.\n\r",ch);
...
RPEDIT (rpedit_create)
{
    ...
    if ( !IS_BUILDER(ch, ad) )
    {
        sendch("RPEdit : Insufficient security to create RoomProgs.\n\r", ch);
        return FALSE;
    }

    if ( get_prog_index(value,PRG_RPROG) )
    {
	sendch("RPEdit: Code vnum already exists.\n\r",ch);
	return FALSE;
    }
    ...
    sendch("RoomProgram Code Created.\n\r",ch);
