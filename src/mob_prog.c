...
int num_eval( int lval, int oper, int rval )
{
    switch( oper )
    {
        ...
        case EVAL_LT:
             return ( lval < rval );
        default:
             logstr (LOG_BUG,  "num_eval: invalid oper", 0 );
             return 0;
    }
}
...
CHAR_DATA *get_random_char( CHAR_DATA *mob, OBJ_DATA *obj, ROOM_INDEX_DATA *room )
{
    ...
    if ( (mob && obj) || (mob && room) || (obj && room) ) {
		logstr (LOG_BUG,  "get_random_char received multiple prog types",0);
		return NULL;
    }
...
int count_people_room( CHAR_DATA *mob, OBJ_DATA *obj, ROOM_INDEX_DATA *room, int iFlag )
{
    ...
    if ( (mob && obj) || (mob && room) || (obj && room) )
    {
	logstr (LOG_BUG,  "count_people_room received multiple prog types",0);
	return 0;
    }
...
int get_order( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    if ( ch && obj )
    {
	logstr (LOG_BUG,  "get_order received multiple prog types",0);
	return 0;
    }
...
bool get_mob_vnum_room( CHAR_DATA *ch, OBJ_DATA *obj, ROOM_INDEX_DATA *room, sh_int vnum )
{
    ...
    {
	logstr (LOG_BUG,  "get_mob_vnum_room received multiple prog types",0);
	return FALSE;
    }
...
bool get_obj_vnum_room( CHAR_DATA *ch, OBJ_DATA *obj, ROOM_INDEX_DATA *room, sh_int vnum )
{
    ...
    if ( (ch && obj) || (ch && room) || (obj && room) )
    {
	logstr (LOG_BUG,  "get_obj_vnum_room received multiple prog types",0);
	return FALSE;
    }
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	case CHK_CLONES:
	    logstr (LOG_BUG,  "cmd_eval_obj: received CHK_CLONES.",0); break;
	case CHK_ORDER:
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	case CHK_CLONES:
	    logstr (LOG_BUG,  "Cmd_eval_room: received CHK_CLONES.",0); break;
	case CHK_ORDER:
	    logstr (LOG_BUG,  "Cmd_eval_room: received CHK_ORDER.",0); break;
	case CHK_HOUR:
	    rval = time_info.hour; break;
	default:;
    }
    ...
    {
    	case 'i':
            logstr (LOG_BUG,  "Cmd_eval_room: received code case 'i'.",0); break;
        case 'n':
            lval_char = ch; break;
        case 't':
            lval_char = vch; break;
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
    ...
    while ( *str != '\0' )
    {
        ...
        switch ( *str )
        {
            default:  logstr (LOG_BUG,  "Expand_arg_mob: bad code %d.", *str );
                          i = " <@@@> ";                        break;
            case 'i':
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
    ...
    if ( obj && room )
    {
	logstr (LOG_BUG,  "expand_arg_other received a obj and a room",0);
	return;
    }
    ...
    while ( *str != '\0' )
    {
        ...
        switch ( *str )
        {
            default:  logstr (LOG_BUG,  "Expand_arg: bad code %d.", *str );
                          i = " <@@@> ";                        break;
            case 'i':
		if ( obj )
		{
		...
		{
		    logstr (LOG_BUG,  "Expand_arg_other: room had an \"i\" case.",0);
		    i = " <@@@> ";
		}						break;
            case 'I':
		if ( obj )
		...
		: someone;					break;
            case 'j':
		logstr (LOG_BUG,  "Expand_arg_other: Obj/room received case 'j'",0);
		i = " <@@@> ";     break;
            case 'e':
	    	i = (ch != NULL )
		? he_she  [URANGE(0, ch->sex, 2)]
		...
		: someone;					break;
            case 'k':
		logstr (LOG_BUG,  "Expand_arg_other: received case 'k'.",0);
		i = " <@@@> ";					break;
            case 'm':
	    	i = (ch != NULL )
		? him_her [URANGE(0, ch->sex, 2)]
		...
		: someone;					break;
            case 'l':
		logstr (LOG_BUG,  "Expand_arg_other: received case 'l'.",0);
		i = " <@@@> ";					break;
            case 's':
	    	i = (ch != NULL )
		? his_her [URANGE(0, ch ->sex, 2)]
...

{
    ...
    if ( (mob && obj) || (mob && room) || (obj && room) )
    {
	logstr (LOG_BUG,  "PROGs: program_flow received multiple prog types.", 0 );
	return;
    }
    ...
    {
	logstr (LOG_BUG,  "PROGs: program_flow did not receive a prog type.", 0 );
	return;
    }
	...
	*b = *c = *d = '\0';
	...
	else if ( (!level || cond[level] == TRUE) && buf[0] != '\0' )
	{
		...
		if ( !mob )
		    logstr (LOG_BUG,  "mob command in non MOBprog",0);
		else
		...
		line = one_argument( data, control );
		if ( !obj )
		    logstr (LOG_BUG,  "obj command in non OBJprog",0);
		else
		...
		line = one_argument( data, control );
		if ( !room )
		    logstr (LOG_BUG,  "room command in non ROOMprog", 0 );
		else
		 ...
		 */
		if ( !mob )
		    logstr (LOG_BUG,  "Normal MUD command in non-MOBprog, prog vnum %d", pvnum );
		else
		    interpret( mob, data );
	    }
	}
	...
	const void *arg1, const void *arg2, int type )
{
    ...
    if ( (mob && obj) || (mob && room) || (obj && room) )
    {
	logstr (LOG_BUG,  "Multiple program types in ACT trigger.", 0 );
	return;
    }
	...
	logstr (LOG_BUG,  "ACT trigger with no program type.", 0 );
	...
	CHAR_DATA *ch, const void *arg1, const void *arg2, int type )
{
    ...
    if ( (mob && obj) || (mob && room) || (obj && room) )
    {
	logstr (LOG_BUG,  "Multiple program types in PERCENT trigger.", 0 );
	return ( FALSE );
    }
	...
	logstr (LOG_BUG,  "PERCENT trigger missing program type.", 0 );
			...
			CHAR_DATA *ch, OBJ_DATA *dropped, int type )
{
    ...
    if ( (mob && obj) || (mob && room) || (obj && room) )
    {
	logstr (LOG_BUG,  "Multiple program types in GIVE trigger.", 0 );
	return;
    }
