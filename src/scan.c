...
void do_scan (CHAR_DATA * ch, char *argument)
{
    ...
    if (get_scouter(ch) == NULL && get_skill(ch, gsn_sense) < 1) {
        sendch ("You do not have a scouter or the sense skill.\n\r", ch);
        return;
    }
    ...
    if (arg1[0] == '\0')
    {
        ...
        sendch ("Looking around you detect:\n\r", ch);
...
void scan_char (CHAR_DATA * victim, CHAR_DATA * ch, sh_int depth, sh_int door)
{
    ...
    sprintf (buf, "An energy source with powerlevel %s", get_pl_from_scouter(ch, victim));
    strcat (buf, ", ");
