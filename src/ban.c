...
void ban_site (CHAR_DATA * ch, char *argument, bool fPerm)
{
    ...
    if (arg1[0] == '\0')
    {
        if (ban_list == NULL)
        {
            sendch ("No sites banned at this time.\n\r", ch);
            return;
        }
        buffer = new_buf ();
    ...
    if (strlen (name) == 0)
    {
        sendch ("You have to ban SOMETHING.\n\r", ch);
        return;
    }
    ...
    for (pban = ban_list; pban != NULL; prev = pban, pban = pban->next)
    {
        if (!str_cmp (name, pban->name))
        {
            ...
            {
                sendch ("That ban was set by a higher power.\n\r", ch);
                return;
            }
            else
            {
...
void do_allow (CHAR_DATA * ch, char *argument)
{
    ...
    if (arg[0] == '\0')
    {
        sendch ("Remove which site from the ban list?\n\r", ch);
        return;
    }
