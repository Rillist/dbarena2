            ...
            && obj->pIndexData->vnum == OBJ_VNUM_WHISTLE))
    {
        ...
        for (vch = char_list; vch != NULL; vch = vch->next)
        {
            ...
            if (vch->in_room != ch->in_room
                ...
                sendch ("You hear a shrill whistling sound.\n\r", vch);
        }
    }
...
bool spec_nasty (CHAR_DATA * ch)
{
    ...
    if (ch->position != POS_FIGHTING)
    {
        for (victim = ch->in_room->people; victim != NULL; victim = v_next)
        {
                ...
                && (victim->level < ch->level + 10))
            {
                if (ch->position != POS_FIGHTING)
                {
                    do_function (ch, &do_attack, victim->name);
                }
    ...
    {
        case 0:
            act ("$n rips apart your coin purse, spilling your zenni!",
                 ch, NULL, victim, TO_VICT);
            act ("You slash apart $N's coin purse and gather his zenni.",
                 ch, NULL, victim, TO_CHAR);
            act ("$N's coin purse is ripped apart!",
                 ch, NULL, victim, TO_NOTVICT);
            zenni = victim->zenni / 10;    /* steal 10% of his zenni */
            victim->zenni -= zenni;
            ch->zenni += zenni;
            return TRUE;
...
{
/* removed to due to removal of spells */
/*
    CHAR_DATA *victim;
    CHAR_DATA *v_next;
    int sn;
...
{
    return TRUE; /* removed to due to removal of spells */ // dragon (ch, "acid breath");
}
...
{
    return TRUE; /* removed to due to removal of spells */ // dragon (ch, "fire breath");
}
...
{
    return TRUE; /* removed to due to removal of spells */ // dragon (ch, "frost breath");
}
...
{
    return TRUE; /* removed to due to removal of spells */ // dragon (ch, "lightning breath");
}
...
{
/* removed dued to removel of magic */
/*
    CHAR_DATA *victim;
    CHAR_DATA *v_next;
...
bool spec_thief (CHAR_DATA * ch)
{
    ...
    for (victim = ch->in_room->people; victim != NULL; victim = v_next)
    {
        ...
        {
            zenni = victim->zenni * UMIN (number_range (1, 20), ch->nDifficulty/10) / 100;
            ch->zenni += zenni;
            victim->zenni -= zenni;
            return TRUE;
        }
