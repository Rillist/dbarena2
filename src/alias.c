...
void substitute_alias (DESCRIPTOR_DATA * d, char *argument)
{
    ...
    if (ch->prefix[0] != '\0' && str_prefix ("prefix", argument))
    {
        if (strlen (ch->prefix) + strlen (argument) > MAX_INPUT_LENGTH - 2)
            sendch ("Line to long, prefix not processed.\r\n", ch);
        else
        {
...
{
    sendch ("I'm sorry, alias must be entered in full.\n\r", ch);
    return;
}
...
void do_alias (CHAR_DATA * ch, char *argument)
{
    ...
    if (arg[0] == '\0')
    {
        ...
        {
            sendch ("You have no aliases defined.\n\r", ch);
            return;
        }
        sendch ("Your current aliases are:\n\r", ch);
    ...
    {
        sendch ("Sorry, that word is reserved.\n\r", ch);
        return;
    }
	...
	{
		sendch("The word to be aliased should not contain a space, "
    ...
    if (argument[0] == '\0')
    {
        ...
        sendch ("That alias is not defined.\n\r", ch);
        return;
    }

    if (!str_prefix (argument, "delete") || !str_prefix (argument, "prefix"))
    {
        sendch ("That shall not be done!\n\r", ch);
        return;
    }
    ...
    if (pos >= MAX_ALIAS)
    {
        sendch ("Sorry, you have reached the alias limit.\n\r", ch);
        return;
    }
...
void do_unalias (CHAR_DATA * ch, char *argument)
{
    ...
    if (!found)
        sendch ("No alias of that name to remove.\n\r", ch);
}
